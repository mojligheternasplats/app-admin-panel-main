...
export const columns = ({ onEdit, onDelete }: ColumnsProps): ColumnDef<Partner, any>[] => [
    {
    accessorKey: 'logoUrl',
    header: () => 'Logo',
    cell: ({ row }) => {
      ...
      return logoUrl ? (
        ...
        <div className="h-10 w-10 rounded-md bg-secondary flex items-center justify-center text-xs text-muted-foreground">
          No Logo
        </div>
      );
    },
  ...
  {
    accessorKey: 'website',
    header: ({ column }) => <Button variant="ghost" onClick={() => column.toggleSorting(column.getIsSorted() === 'asc')}>Website</Button>,
    cell: ({ row }) => (
      <a href={row.getValue('website')} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction onClick={() => onDelete(partner.id)}>Confirm</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
